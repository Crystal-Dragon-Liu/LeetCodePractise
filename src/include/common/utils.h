...
// The pointer should be initialized 
...
class BSTIterator {
public:
    BSTIterator(TreeNode* root): idx(0), arr(inorder(root)){} // because of the inorder traversal, the first index is the smallest one.
    int next(){ return arr[idx++];}   
    bool hasNext(){return idx < static_cast<int>(arr.size());}
    std::vector<int>  inorder(TreeNode* root)
    {
        std::vector<int> ret;
        updateInorder(root, ret);
        return ret;
    }
    void updateInorder(TreeNode* root, std::vector<int>& ret)
    {
        if(!root) return;
        if(root->left) updateInorder(root->left, ret);
        ret.push_back(root->val);
        if(root->right) updateInorder(root->right, ret);
    }
private:
    std::vector<int> arr;
    int idx;
